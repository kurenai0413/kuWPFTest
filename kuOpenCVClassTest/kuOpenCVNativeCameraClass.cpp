...
bool kuOpenCVNativeCameraClass::kuOpenCVNativeCameraClassImpl::kuGenerateHairMask()
{
	...
	else
	{
		...
		m_PaddingStartX = paddingROI.x;
		m_PaddingStartY = paddingROI.y;
...
void kuOpenCVNativeCameraClass::kuOpenCVNativeCameraClassImpl::kuSetHairHueColor(int hueValue)
{
	m_HueValue = hueValue;
}
...
#pragma endregion
	...
	for (int j = 0; j < finalHairMask.rows; j++)
	{
		for (int i = 0; i < finalHairMask.cols; i++)
		{
			int idxInOriX = i + m_PaddingStartX;
			int idxInOriY = j + m_PaddingStartY;

			int BVal = (int)finalHairMask.at<uchar>(j, 3 * i);
			int GVal = (int)finalHairMask.at<uchar>(j, 3 * i + 1);
			int RVal = (int)finalHairMask.at<uchar>(j, 3 * i + 2);

			int SVal = (int)m_UpdatedHSVImg.at<uchar>(idxInOriY, 3 * idxInOriX + 1);
			int VVal = (int)m_UpdatedHSVImg.at<uchar>(idxInOriY, 3 * idxInOriX + 2);

			if ((BVal == 255) && (GVal == 255) && (RVal == 255))
			{
				m_UpdatedHSVImg.at<uchar>(idxInOriY, 3 * idxInOriX)		= m_HueValue;
				m_UpdatedHSVImg.at<uchar>(idxInOriY, 3 * idxInOriX + 1) = 1.5 * SVal;
				m_UpdatedHSVImg.at<uchar>(idxInOriY, 3 * idxInOriX + 2) = 1.5 * VVal;
			}
		}
